...
2. **Show evidence** when available.
3. **Diagnose** the issue with AI.
4. **Ask targeted questions** when context is missing.
5. **Follow guided steps** toward resolution.
6. **Mark the case resolved** and retain its state.
...
For deployed Lambda, use the model identifier available to the account/region. The production deployment may use a Bedrock inference-profile/model identifier rather than the local default.
...
2. A YouTube demo of up to three minutes
3. A short write-up covering the problem, build and AWS usage
